# -*- coding: utf-8 -*-
"""COVIDScholar_multilabel_distilbert_benchmark

# Fine Tuning DistilBERT for COVID-19 Document MultiLabel Text Classification
"""
...
tokenizer = AutoTokenizer.from_pretrained("allenai/specter", truncation=True, do_lower_case=True)
...
# Creating the customized model, by adding a drop out and a dense layer on top of specter to get the final output for the model. 
class SpecterClass(torch.nn.Module):
    def __init__(self):
        super(SpecterClass, self).__init__()
        self.l1 = AutoModel.from_pretrained("allenai/specter")
...
with open('specter_benchmark_results.json', 'w') as f:
    json.dump(output, f)